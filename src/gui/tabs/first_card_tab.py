...
class FirstCardTab:
    ...
    def run_validation(self):
        ...
        try:
            ...
            if operator == "JIO":
                # JIO validation logic (existing functionality)
                ...
                # NEW: Verify profile matches PCOM file content
                if paths[1]:  # If PCOM file is selected
                    self.log_output.delete(1.0, tk.END)
                    ...
                    if not profile_match:
                        ...
                        pcom_filename = os.path.basename(paths[1])
                        pcom_dir = os.path.dirname(paths[1])
                        
                        self.log_output.insert(tk.END, f"❌ PROFILE VERIFICATION FAILED!\n")
                        self.log_output.insert(tk.END, "=" * 50 + "\n")
                        self.log_output.insert(tk.END, f"Selected Profile : {profile}\n")
                        self.log_output.insert(tk.END, f"PCOM File        : {pcom_filename}\n")
                        self.log_output.insert(tk.END, f"File Location    : {pcom_dir}\n")
                        self.log_output.insert(tk.END, f"Error Details    : {profile_message}\n")
                        self.log_output.insert(tk.END, "=" * 50 + "\n\n")
                        self.log_output.insert(tk.END, "❌ VALIDATION BLOCKED: Profile mismatch detected!\n\n")
                        self.log_output.insert(tk.END, "Recommended Action:\n")
                        self.log_output.insert(tk.END, "1. Check if you selected the correct profile type\n")
                        self.log_output.insert(tk.END, "2. Verify the PCOM file matches the selected profile\n")
                        self.log_output.insert(tk.END, "3. If needed, select a different PCOM file\n")
                        self.log_output.insert(tk.END, "4. Click 'Clear All' and start over if necessary\n")
                        
                        # FIXED: Show error message and block validation completely
                        messagebox.showerror(
                            "Profile Mismatch Detected",
                            f"PROFILE VERIFICATION FAILED!\n\n"
                            f"Selected Profile: {profile}\n"
                            f"PCOM File: {pcom_filename}\n\n"
                            f"Error: {profile_message}\n\n"
                            f"The selected profile does not match the PCOM file content.\n\n"
                            f"VALIDATION CANNOT PROCEED!\n\n"
                            f"Please:\n"
                            f"1. Verify you selected the correct profile\n"
                            f"2. Check if the PCOM file matches '{profile}' profile\n"
                            f"3. Restart validation with correct files"
                        )
                        
                        # Reset flag and return to block validation
                        self._validation_in_progress = False
                        return
                    else:
                        self.log_output.insert(tk.END, f"✅ {profile_message}\n")
                        self.log_output.insert(tk.END, f"✅ Profile '{profile}' verified successfully.\n\n")

                try:
                    # Import validation engine
                    from first_card_validation.core.validation_engine import main as run_validation_engine  # type: ignore
                    
                    self.log_output.insert(tk.END, "Starting JIO validation...\n")
                    self.log_output.update()
                    ...
                    # CRITICAL FIX: Check if report was actually saved
                    if not report_path:
                        self.log_output.insert(tk.END, "❌ Validation failed to generate report!\n")
                        if validation_errors:
                            for error in validation_errors:
                                if "Permission denied" in str(error) or "Errno 13" in str(error):
                                    self.log_output.insert(tk.END, f"❌ File permission error: {error}\n")
                                    self.log_output.insert(tk.END, "The report file cannot be saved because it's open or locked.\n")
                                    self.log_output.insert(tk.END, "Please close any open Excel files and try again.\n")
                                    messagebox.showerror(
                                        "Report File Open",
                                        "Cannot save the report because the file is open or locked.\n\n"
                                        "Please close the Excel file and try again:\n"
                                        f"Check if this file is open: {paths[0]}_Validation_Report.xlsx"
                                    )
                                    return
                        ...
                        messagebox.showerror("Report Generation Failed", "Cannot save the report because the Excel file is currently open. Please close the report file and try again.")
                        return
                    
                    # Check if the report file exists and is accessible
                    if report_path and os.path.exists(report_path):
                        # Check if the newly created report file is now open (prevent overwrite issues)
                        if is_file_open(report_path):
                            self.log_output.insert(tk.END, f"⚠️  Warning: Report file is now open: {report_path}\n")
                            self.log_output.insert(tk.END, "Please close it before running validation again.\n\n")
                        
                        self.log_output.insert(tk.END, f"✅ JIO Validation completed successfully.\n")
                        ...
                        if validation_errors:
                            ...
                            self.log_output.insert(tk.END, "✅ All data fields validated successfully!\n")
                        ...
                        # Report path returned but file doesn't exist
                        self.log_output.insert(tk.END, f"❌ Report file not found at: {report_path}\n")
                        self.log_output.insert(tk.END, "Validation may have completed but report was not saved.\n")
                    ...
                    if "Permission denied" in str(e) or "Errno 13" in str(e):
                        error_msg += "❌ File permission error. The report file may be open in another program.\n"
                        error_msg += "Please close any open Excel files and try again.\n"
                ...
                # Note: AIRTEL doesn't have profile selection in GUI, but we can still check PCOM content
                ...
                print(f"📸 AIRTEL Image paths collected: {airtel_image_paths}")
                
                # Debug: Print the paths to see what we're getting
                print(f"🔍 AIRTEL Debug - ML: '{ml_path}'")
                print(f"🔍 AIRTEL Debug - PCOM: '{pcom_path}'")
                print(f"🔍 AIRTEL Debug - CNUM: '{cnum_path}'")
                print(f"🔍 AIRTEL Debug - SIM ODA: '{sim_oda_path}'")
                print(f"🔍 AIRTEL Debug - Images: {len(airtel_image_paths)} provided")
                ...
                # Note: PCOM is optional for Airtel, so we don't check if it's missing
                ...
                if missing_files:
                    ...
                    # Add debug info
                    error_message += f"\nDebug Info:\n"
                    error_message += f"Machine Log: {ml_path or 'Not selected'}\n"
                    error_message += f"CNUM: {cnum_path or 'Not selected'}\n" 
                    error_message += f"SIM ODA: {sim_oda_path or 'Not selected'}"
                    
                    messagebox.showerror("File Selection Error", error_message)
                    return

                try:
                    # Try to import Airtel validation module
                    try:
                        from first_card_validation.core.airtel_validation import main_airtel as run_airtel_validation  # type: ignore
                    except ImportError as e:
                        self.log_output.insert(tk.END, f"❌ AIRTEL validation module not found: {str(e)}\n")
                        self.log_output.insert(tk.END, "Please ensure airtel_validation.py is in the correct location.\n")
                        messagebox.showerror("Module Error", "AIRTEL validation module not found. Please check the installation.")
                        return
                    ...
                    # Run Airtel validation WITH image_paths - FIXED
                    report_path, validation_errors = run_airtel_validation(
                        filepath=ml_path,      # Machine Log
                        pcom_path=pcom_path,   # PCOM (optional for Airtel)
                        cnum_path=cnum_path,   # CNUM
                        sim_oda_path=sim_oda_path,  # SIM ODA
                        image_paths=airtel_image_paths  # FIXED: Pass image paths here
                    )

                    # CRITICAL FIX FOR AIRTEL TOO: Check if report was actually saved
                    if not report_path:
                        ...
                        if validation_errors:
                            for error in validation_errors:
                                if "Permission denied" in str(error) or "Errno 13" in str(error):
                                    ...
                                    messagebox.showerror(
                                        "Report File Open",
                                        "Cannot save the AIRTEL report because the file is open or locked.\n\n"
                                        "Please close any open Excel files and try again."
                                    )
                                    return
                        
                        self.log_output.insert(tk.END, "❌ Report generation failed. Check the validation engine logs.\n")
                        if validation_errors:
                            self.log_output.insert(tk.END, "Errors returned:\n")
                            for error in validation_errors:
                                self.log_output.insert(tk.END, f"• {error}\n")
                    ...
                    # Check if the report file exists and is accessible
                    if report_path and os.path.exists(report_path):
                        # Check if the newly created report file is now open
                        if is_file_open(report_path):
                            self.log_output.insert(tk.END, f"⚠️  Warning: Report file is now open: {report_path}\n")
                            self.log_output.insert(tk.END, "Please close it before running validation again.\n\n")
                        ...
                        # Show image info in log
                        if airtel_image_paths:
                            image_count = len([path for path in airtel_image_paths.values() if path])
                            self.log_output.insert(tk.END, f"📸 Image paths provided: {image_count}\n")
                            if image_count > 0:
                                self.log_output.insert(tk.END, f"📸 Images should be inserted in the Excel report\n")
                        ...
                        if validation_errors:
                            ...
                            self.log_output.insert(tk.END, f"Total errors found: {len(validation_errors)}\n\n")
                            messagebox.showwarning("Validation Completed with Errors", 
                                                f"AIRTEL validation completed with {len(validation_errors)} data field error(s).\nCheck the log for details.")
                        else:
                        ...
                        # Report path returned but file doesn't exist
                        self.log_output.insert(tk.END, f"❌ Report file not found at: {report_path}\n")
                        self.log_output.insert(tk.END, "AIRTEL validation may have completed but report was not saved.\n")
                        messagebox.showerror("AIRTEL Report Not Found", "AIRTEL report file was not created. Check permissions and disk space.")

                    self.log_output.see(1.0)

                except Exception as e:
                    error_msg = f"❌ AIRTEL Validation Error: {str(e)}\n"
                    # Check for permission errors
                    if "Permission denied" in str(e) or "Errno 13" in str(e):
                        error_msg += "❌ File permission error. The report file may be open in another program.\n"
                        error_msg += "Please close any open Excel files and try again.\n"
                    ...
                    self.log_output.insert(tk.END, error_msg)
                    import traceback
                    self.log_output.insert(tk.END, f"Detailed error:\n{traceback.format_exc()}\n")
    ...
    def get_icon_path(self):
        ...
        try:
            ...
            if os.path.exists(icon_path):
                print(f"First Card - Icon found: {icon_path}")
                return icon_path
            print("First Card - Icon not found")
            return None
        except Exception as e:
            print(f"First Card - Error finding icon: {e}")
            return None
    ...
    def create_form_elements(self):
        ...
        self.profile_cb.set("Select profile")
        self.profile_cb.grid(row=row, column=1, pady=4, padx=4, sticky="w", columnspan=2)
        self.operator_widgets['jio'].extend([profile_label, self.profile_cb])
        ...
        if len(jio_entries) >= 10:
            ...
            print("✅ JIO entries assigned successfully")
        ...
        if len(airtel_entries) >= 6:  # Changed from 4 to 6 (4 files + 2 images)
            ...
            if len(airtel_image_entries) >= 2:
                ...
                print("✅ Airtel image entries assigned successfully")
            
            print("✅ Airtel-specific entries assigned successfully")
            ...
            print(f"❌ Not enough Airtel entries: {len(airtel_entries)}")
